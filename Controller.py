...
class Controls():
	...
	def __init__(self):
		...
		dbpath = 'C:\\SciData'
		dbname = 'HRT_6500_Validation.db'
		
		self.sciDB = SciDatabase(os.path.join(dbpath, dbname))
	...
	def ETL(self):
		...
		csvExtractor = Extract(self.csvDirectory, self.sciDB.schema, self.DebuggingMode)
	...
	def CreateSpectralQualityVisualizations(self):
		...
		for n, analyte_table in enumerate(analyte_table_lst):
			...
			self.calc.BuildSimilarity_PlotsTables(similarity_df, data_sets, analytes_lst[n])
	...
	def CreateSummaryTable(self, data_set):
		...
		for analyte_table in analyte_table_lst:
			...
			if ProcessingType == "TargetAnalyteFinding":
				...
				IDL = self.calc.calculate_IDL(Area_100fg_lst, 100)
